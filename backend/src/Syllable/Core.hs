...
-- Right [Syllable [...], ...]  -- silabação pré-definida
--
-- >>> syllabify "aero-espacial"  -- palavra composta
-- Right [Syllable [...], Syllable [...], ...]  -- silabação de cada parte
  ...
  let word = normalize text
   in case Map.lookup word exceptionMap of
        ...
        Nothing -- Rule-based path: apply phonological processing
         ...
         ->
          let parts = splitOnHyphen word
           in if length parts > 1
                then do
                  syllabifiedParts <- mapM syllabifyPart parts
                  return (concat syllabifiedParts)
                else syllabifyPart word

-- | Syllabify a single word part (without hyphens)
syllabifyPart :: String -> Either String [Syllable]
syllabifyPart word =
  let phonemes = toPhonemes word
   in applyRules phonemes

-- | Split a string on hyphens, removing empty parts
splitOnHyphen :: String -> [String]
splitOnHyphen = filter (not . null) . splitOn '-'
  where
    ...
    splitOn c (x:xs)
      | x == c = "" : splitOn c xs
      | otherwise =
        case splitOn c xs of
          (ys:yss) -> (x : ys) : yss
          []       -> [x : ""] -- This shouldn't happen, but to be safe
